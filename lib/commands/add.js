...
export async function addCommand(args) {
  ...
  if (await isLocalPathTarget(rawTarget)) {
    await addLocalCommand(rawTarget, { specificTemplate, installAll });
    return;
  }

  const target = parseInstallTarget(rawTarget);
  if (!target) {
  ...
  try {
    ...
    const installed = await installTemplateDirs({
      templateDirs,
      repoName: branchInfo.fullName,
      repoRef: branchInfo.branch,
      sha: sha || '',
      sourceLabel: branchInfo.fullName,
      track: true
    });
...
async function addLocalCommand(targetPath, { specificTemplate, installAll }) {
  const rootPath = resolveLocalPath(targetPath);

  console.log(dim(`Resolving ${rootPath}...`));
  ...
  try {
    ...
    console.error(red(`Error: Local template path not found: ${targetPath}`));
    process.exit(1);
  }
  ...
  const installed = await installTemplateDirs({
    templateDirs,
    repoName: `local:${rootRealPath}`,
    repoRef: 'local',
    sha: '',
    sourceLabel: rootRealPath,
    track: false
  });
...
async function installTemplateDirs({ templateDirs, repoName, repoRef, sha, sourceLabel, track }) {
  let installed = 0;

  for (const template of templateDirs) {
    const result = await validateTemplate(template.path);
    if (!result.valid) {
      console.error(red(`\nValidation failed for ${template.relativePath || sourceLabel}:`));
    ...
    const id = result.meta.id || template.name || basename(sourceLabel);
    const destDir = join(TEMPLATES_DIR, id);
    await rm(destDir, { recursive: true, force: true });
    await mkdir(destDir, { recursive: true });
    await cp(template.path, destDir, { recursive: true });

    const installTarget = buildInstallTarget(sourceLabel, template.relativePath);
    const source = {
      repo: repoName,
      ref: repoRef,
      sha,
    ...
    if (track) {
      trackInstall(repoName, id, template.relativePath, installTarget).catch(() => {});
    }
    installed += 1;
  }
...
async function isLocalPathTarget(target) {
  if (!target) {
    return false;
  }
  ...
  try {
    const stats = await stat(resolve(target));
...
async function findTargetsFromRequestedPath(tmpDir, requestedPath, specificTemplate, installAll) {
  ...
  const manifestMatch = await findTargetFromRootManifest(tmpDir, requestedPath);
  if (manifestMatch) {
...
async function findTargetFromRootManifest(tmpDir, requestedPath) {
  const requestedName = basename(trimSlashes(requestedPath));
  if (!requestedName) {
    return null;
  }

  const targets = await getManifestTargets(tmpDir, '', requestedName, false);
